...
- [ ] 编写测试，覆盖帧发布序列、健康端点的新鲜/过期状态、快照 JPEG 和
  MJPEG multipart 帧。
- [ ] 运行
  `python -m unittest discover -s tests/camera -p "test_*.py" -v`。
- [ ] 确认因 `camera.orin_camera_stream` 不存在而失败。
...
- [ ] 实现 `FrameBuffer`，记录 sequence、最新帧和单调时钟时间。
- [ ] 实现不依赖硬件的 HTTP handler 和 `CameraHttpServer`。
- [ ] 保持 `/healthz`、`/snapshot.jpg`、`/stream.mjpg` 路径兼容。
- [ ] 实现 GStreamer MJPEG appsink pipeline 和 bus 错误退出。
- [ ] 运行 Task 1 测试并确认全部通过。
- [ ] 运行 `python -m py_compile camera/orin_camera_stream.py`。
...
- [ ] unit 使用仓库脚本、固定 by-id 图像节点、1280x720@60 采集和 15 fps
  输出。
- [ ] 安装脚本验证用户、设备路径、Python GI/GStreamer，备份旧部署后原子
  安装 unit。
- [ ] 把 `media-gateway` 上游改为
  `http://127.0.0.1:8090/stream.mjpg`，更新过期设备别名。
- [ ] 用 `bash -n scripts/install_camera_stream.sh`、
...
- [ ] 提交并推送 feature 分支。
- [ ] 将提交同步到 Nano 的 `develop`，保留未跟踪 telemetry 文件。
- [ ] 执行安装脚本；仅相机流会短暂中断。
- [ ] 验证 unit 的 `ExecStart` 已指向仓库，旧 `/opt` 进程已退出。
...
- [ ] 核对 `fuser /dev/video0` 只有一个服务 PID。
- [ ] 验证健康、快照、两个并发 MJPEG 客户端和 unit 重启恢复。
- [ ] 检查 1280x720 快照平均亮度及帧内容。
- [ ] 核对飞行三个容器健康和关键 ROS 数据仍存在。
- [ ] 记录备份路径、部署提交、服务状态、端点和 RK3588 接入地址。
- [ ] 提交并推送最终证据。
