...
## 2026-07-26 官方赛题域闭集模型最终结论

- 先前真实开放数据子集的 73.68% 召回率只能说明 iNaturalist 弱标注域与赛题俯视、
  旋转、地貌域不一致；该小样本结果不能直接代表 2025 H 题验收性能。最终训练改用
  官方动物体态图和地貌图构造严格闭集数据：train 6000、val 800、test 1200，独立
  test 含 1953 个目标、551 张多实例场景和 488 张空场景。
- 首次启动失败是数据集原子移动后 `dataset.yaml` 仍指向 `dataset.tmp`；在 GPU 尚未
  开始训练前修正路径。AMP 自检尝试重复下载 `yolo26n.pt` 时也在首轮前停止，复用
  已校验的本地权重。两次均未污染正式实验结果。
- `lr0=0.001` 的首轮在第 2--3 epoch 已出现验证指标退化，因此及时终止。正式低学习
  率阶段采用 `lr0=0.00015`，第 2 epoch 达到最佳；随后从 2411 张训练空场景挖出
  20 张难负样本并 4 倍过采样，以 `lr0=0.00005` 做 4 epoch 定向收尾。
- 最终 hard-negative 模型在独立 test 上：box mAP50-95 `0.94335`、mask mAP50-95
  `0.83664`。`conf=0.70` 时目标召回 `99.03%`、预测精确率 `99.59%`、多实例整场
  全命中率 `97.28%`，空场景误报 `1/488`（`0.205%`）。比定向收尾前的
  `13/488` 空场景误报减少 `92.3%`。
- 提高到 `conf=0.90` 会把召回率压到 `87.20%`，却仍无法消除最后一张误报，因此
  单纯提高置信度不是正确解法。最后误报 `scene-00219.jpg` 是数据生成器添加的半透明
  灰色矩形伪影，不是官方地貌图自然纹理；部署时使用几何约束和时序确认处理。
- 最终输出使用非阻塞滑动窗口：候选框立即显示；`conf >= 0.85` 且通过几何检查的
  目标采用 2/3 帧快速确认，`0.70 <= conf < 0.85` 采用 3/5 帧确认。已确认轨迹只做
  连续跟踪，不重复等待 5 帧；因此 10 FPS 下从首次命中到确认约为 0.1/0.2 秒，
  不会阻塞采集或推理线程。
- 最终权重：
  `C:\nano\state\vision-training\yolo11s-seg-nuedc-h-official-v2-20260726\best-hardneg.pt`，
  20,532,132 bytes，SHA-256
