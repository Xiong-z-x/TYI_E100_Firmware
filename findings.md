...
- 用户随后确认该次模糊源于镜头硬件焦距调整，并要求恢复 `sharpness=4`。此次
  ...
  相互独立。
- 30 FPS 是服务端上限而不是无线客户端必达值。当前 Windows 单客户端实测
  23.2 FPS；第二个并发MJPEG客户端降到约6.7 FPS，证明主要限制是同一Wi-Fi链路
  上的重复JPEG传输。竞赛运行时应由单一视频消费者接收画面，再在地面站内部
  分发，不能让多个进程各自从Nano重复拉流。
- 当前闭集门槛确实过度偏向精度。独立合成集的阈值扫描证明
  `0.60/0.20/0.15 + 连续3帧`是预训练阶段更合理的临时工作点：目标召回提高
  12个百分点，接受精度只下降0.33个百分点，且该合成unknown集没有新增误报。
  这仍需用真实空地图验证，不能直接当作最终比赛阈值。

## 2026-07-26 AutoDL 多实例训练结论

- Open Images V7 的实例分割类别不能完整覆盖赛题五类，因此没有伪称五类都具备
  人工实例掩膜；统一改用 iNaturalist 许可过滤图像、MegaDetector 动物框和 SAM2
  掩膜的可复现弱监督路线。
- iNaturalist 公共 API 在深分页时返回 403；下载器将候选页限制在前 10000 条允许
  的 50 页内，并以固定随机种子抽取不同时间段样本，避免重复失败和偏向单一近况。
- MegaDetector 官方权重直连 GitHub 时仅约几十 KB/s；只在该下载 shell 临时执行
  `source /etc/network_turbo` 后完成下载。未持久修改 Windows、WSL 或 AutoDL 的
  代理/DNS/路由配置。
- Ultralytics 启动时还会为 AMP 自检下载 `yolo26n.pt`，首轮因此 GPU 利用率为 0；
  终止未产生训练结果的进程，临时加速补齐该 5.3 MB 权重后恢复正常训练。
- 首次后台重启失败原因为非登录 shell 找不到 `python`；多实例转换首次重启失败
  原因为 MegaDetector 环境不含 Ultralytics。最终均通过显式选择对应虚拟环境解决，
  不修改系统 Python。
- 单实例弱标签会把同图其他动物错误当成背景。对 MegaDetector 结果复核发现 135 张
  图含至少两个高置信动物框、共有 233 个额外候选框，因此在正式训练前改为一图多框
  批量 SAM 提示；严格面积和掩膜/框比例过滤后保留 648 图、753 实例。
