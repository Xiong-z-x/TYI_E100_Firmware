...
- 随后确认模糊来自镜头硬件焦距调整，而不是软件锐度。按用户决定重新启用
  `sharpness=4`，并将局域网原始 MJPEG 从 15 FPS 提升到 30 FPS；SpeciesNet
  推理仍保持 2 FPS，避免为了显示流畅度占用飞行链路的推理余量。
- 30 FPS 配置下，Windows 单客户端连续 5 秒实际读取 116 帧，即 23.2 FPS；
  相比原先约 14.7 FPS 明显改善。并发打开第二个拉流客户端时会争抢 Wi-Fi
  带宽，因此测试和正式显示只保留一个查看器。

## 2026-07-26

- 基于 500 个目标和 100 个 unknown 的独立合成测试重新扫阈值：原配置
  `0.80/0.45/0.30` 的目标召回为 50.8%、接受精度 98.45%；改为
  `0.60/0.20/0.15` 后召回为 62.8%、接受精度 98.12%，unknown 仍为零误报。
- 为抵消降阈值风险，将确认条件从连续 2 帧提高到连续 3 帧；候选框可以更早
  显示，但只有稳定目标进入事件和计数。
- 旋转分桶确认当前 SpeciesNet 域退化：绝对旋转 0--30°召回 90.2%，
  90--120°为 35.1%，150--180°仅 18.4%。降阈值不是最终解决方案，后续改用
  全角度、多实例五类分割模型作为主检测链路。
- AutoDL 训练节点实测为 RTX 4080 SUPER 32 GB、PyTorch 2.8.0+cu128；训练与
  MegaDetector 预处理使用隔离虚拟环境，避免检测器的旧依赖污染 Ultralytics。
- 新增 1260 张 iNaturalist Open Data 真实动物图，限定 `CC0`/`CC-BY` 且仅接受
  `inaturalist-open-data` 图像域；MegaDetector v5a 完成动物框筛选，SAM2.1-s
  将合格框转换为实例分割掩膜。
- 修复首版转换只保留单个最大动物框的问题：真实数据最终保留 648 张图、753 个
  实例，其中 105 个是同图额外目标。合并后数据集共 4048 张图；逐图解码、标签
  坐标/类别、图像标签配对均为 0 错误，train/val/test 间为 0 个哈希重复。
- 正式训练采用 YOLO11s-seg、768 输入、batch 64、80 epochs、全角度旋转与几何/
  模糊增强。AutoDL 后台日志为 `logs/train-yolo11s-seg.log`，训练完成后脚本会自动
  用独立 test split 评估 `best.pt`。
- Windows 已关闭本地实时识别窗口，并按白名单删除旧数据集、冒烟训练输出、旧
  AutoDL 包和临时权重，约释放 1.1 GB；保留已校验上传的最终离线包、仓库、飞控
  数据与可复现实验所需环境。
- 已完成官方赛题域 v2 数据集：6000 train、800 val、1200 独立 test；覆盖五类、
  0--360 度旋转、1--5 个多实例、模糊/缩放以及 40% 左右空场景。
- 高学习率试验在 3 epoch 内确认退化后提前停止；低学习率正式训练早停于 6 epoch，
  再用 20 张挖掘难负样本完成 4 epoch 收尾，避免继续消耗 AutoDL 算力。
- 最终模型独立 test：box mAP50-95 `94.33%`、mask mAP50-95 `83.66%`；在
  `conf=0.70` 下召回 `99.03%`、精确率 `99.59%`、空场景误报 `1/488`、多实例
  整场全命中 `97.28%`。最终权重和全部评估 JSON 已下载到
