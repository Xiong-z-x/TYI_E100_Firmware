...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    if (target_type_ < 0)
    {
      const char *env_flight_type = std::getenv("TYI_PLANNER_FLIGHT_TYPE");
      if (env_flight_type != nullptr)
      {
        target_type_ = std::atoi(env_flight_type);
        ROS_WARN("fsm/flight_type missing in rosparam, fallback to TYI_PLANNER_FLIGHT_TYPE=%d", target_type_);
      }
    }
    ...
    planner_status_pub_ = nh.advertise<std_msgs::String>("/tyi_planner/status", 10, true);
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      if ((target_type_ == TARGET_TYPE::PRESET_TARGET) &&
          ...
          actual_goal_distance < no_replan_thresh_)
      {
        wp_id_++;
        planNextWaypoint(wps_[wp_id_]);
      }
      ...
      {
        if (actual_goal_distance < no_replan_thresh_ && t_cur > info->duration_ - 1e-2)
        {
          have_target_ = false;
          have_trigger_ = false;
        ...
        else if (actual_goal_distance > no_replan_thresh_ && t_cur > replan_thresh_)
        {
          changeFSMExecState(REPLAN_TRAJ, "FSM");
        }
      }
  ...
  bool EGOReplanFSM::planFromCurrentTraj(const int trial_times /*=1*/)
  {
    ...
    ros::Time time_now = ros::Time::now();
    double t_cur = (time_now - info->start_time_).toSec();
    double nominal_t = std::max(0.0, std::min(info->duration_, t_cur));
    const Eigen::Vector3d nominal_pos = info->position_traj_.evaluateDeBoorT(nominal_t);
    const Eigen::Vector3d nominal_vel = info->velocity_traj_.evaluateDeBoorT(nominal_t);
    const Eigen::Vector3d nominal_acc = info->acceleration_traj_.evaluateDeBoorT(nominal_t);
    const double tracking_reset_tolerance = std::max(no_replan_thresh_, 0.35);
    const bool should_reset_from_odom =
        t_cur >= info->duration_ || (odom_pos_ - nominal_pos).norm() > tracking_reset_tolerance;

    if (should_reset_from_odom)
    {
      start_pt_ = odom_pos_;
      start_vel_ = odom_vel_;
      start_acc_.setZero();
    ...
    {
      start_pt_ = nominal_pos;
      start_vel_ = nominal_vel;
      start_acc_ = nominal_acc;
    }

    bool success = callReboundReplan(should_reset_from_odom, false);
  ...
  void EGOReplanFSM::publishPlannerStatus()
  {
    ...
    const bool idle_state = exec_state_ == WAIT_TARGET || exec_state_ == INIT;
    const auto &status = planner_manager_->getLastReplanStatus();
    const auto &local_data = planner_manager_->local_data_;

    std::ostringstream payload;
    payload << std::boolalpha << std::fixed << std::setprecision(4)
            << "{"
            << "\"plannerState\":\"" << state_name << "\",";
    ...
    if (idle_state)
    {
      ...
      {
        payload << "\"goalDistance\":" << (end_pt_ - odom_pos_).norm() << ","
                << "\"target\":{\"x\":" << end_pt_(0) << ",\"y\":" << end_pt_(1) << ",\"z\":" << end_pt_(2) << "},";
      }
      else
      {
        payload << "\"goalDistance\":null,"
                << "\"target\":null,";
      }

      if (have_odom_)
      {
        payload << "\"start\":{\"x\":" << odom_pos_(0) << ",\"y\":" << odom_pos_(1) << ",\"z\":" << odom_pos_(2) << "},";
      }
      else
      {
        payload << "\"start\":null,";
      ...
      payload << "\"trajId\":null,"
              << "\"trajDurationSec\":null,";
    }
    else
    {
      payload << "\"success\":" << status.success << ","
              << "\"planSuccess\":" << status.plan_success << ","
              << "\"refineSuccess\":" << status.refine_success << ","
              << "\"closeToGoal\":" << status.close_to_goal << ","
              << "\"terminalInObstacle\":" << status.terminal_in_obstacle << ","
      ...
      {
        payload << "\"goalDistance\":" << status.goal_distance << ","
                << "\"target\":null,";
      }

      payload << "\"start\":{\"x\":" << start_pt_(0) << ",\"y\":" << start_pt_(1) << ",\"z\":" << start_pt_(2) << "},";

      if (local_data.start_time_.toSec() > 1e-5)
      {
        payload << "\"trajId\":" << local_data.traj_id_ << ","
                << "\"trajDurationSec\":" << local_data.duration_ << ",";
      }
      else
      {
        payload << "\"trajId\":null,"
    ...
    payload << "\"stampSec\":" << ros::Time::now().toSec()
            << "}";

    std_msgs::String planner_status;
    planner_status.data = payload.str();
