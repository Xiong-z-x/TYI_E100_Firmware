...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    if (pp_.drone_id < 0)
    {
      const char *env_drone_id = std::getenv("TYI_PLANNER_DRONE_ID");
      if (env_drone_id != nullptr)
      {
        pp_.drone_id = std::atoi(env_drone_id);
        ROS_WARN("manager/drone_id missing in rosparam, fallback to TYI_PLANNER_DRONE_ID=%d", pp_.drone_id);
      }
    }
  ...
  {
    const double goal_distance = (start_pt - local_target_pt).norm();
    last_replan_status_ = ReplanStatus();
    last_replan_status_.failure_reason = "pending";
    last_replan_status_.goal_distance = goal_distance;
    ...
    if (goal_distance < 0.2)
    {
      ...
      last_replan_status_.close_to_goal = true;
      last_replan_status_.failure_reason = "close_to_goal";
      return false;
    }
        ...
        if (poly_time > ts)
        {
          ...
          for (t = ts; t < poly_time; t += ts)
          {
            ...
            else
            {
              ...
              continous_failures_count_++;
              last_replan_status_.failure_reason = "empty_pseudo_arc_length";
              return false;
            }
          }
        }
    ...
    if (!flag_step_1_success)
    {
      ...
      continous_failures_count_++;
      last_replan_status_.failure_reason = bspline_optimizer_->stoppedForError()
          ? "terminal_in_obstacle"
          : (bspline_optimizer_->stoppedForRebound() ? "collision_rebound_limit" : "optimize_failed");
      last_replan_status_.terminal_in_obstacle = bspline_optimizer_->stoppedForError();
      return false;
    }
    last_replan_status_.plan_success = true;
    last_replan_status_.refine_success = true;
    ...
    if (pp_.drone_id <= 0)
    {
      ...
      if (!flag_step_2_success)
      {
        ...
        continous_failures_count_++;
        last_replan_status_.refine_success = false;
        last_replan_status_.failure_reason = bspline_optimizer_->stoppedForError()
            ? "terminal_in_obstacle"
            : "refine_failed";
        last_replan_status_.terminal_in_obstacle = bspline_optimizer_->stoppedForError();
    ...
    last_replan_status_.success = true;
    last_replan_status_.plan_success = true;
    last_replan_status_.refine_success = true;
    last_replan_status_.failure_reason = "none";
    return true;
  }
